...
import { Gesture, GestureDetector, GestureHandlerRootView, Text } from 'react-native-gesture-handler'
import Animated, { useAnimatedStyle, useSharedValue, useAnimatedRef, scrollTo, withTiming } from 'react-native-reanimated'

function NewLayout(): React.JSX.Element {
  ...
  const panGesture = Gesture.Pan()
    ...
    .onChange(event => {
      ...
      if (scrollY.value <= 0 && translationYStep > 0) {
        ...
        // Swiping down
        translateY.value = Math.min(0, translateY.value + translationYStep * 1.5)
      } else if (translationYStep < 0 && translateY.value > -250) {
        console.log('swipeup')
        // Swiping up
        translateY.value = Math.max(-250, translateY.value + translationYStep * 1.5)
  ...
  return (
    <View style={{ flex: 1 }}>
      <GestureHandlerRootView style={{ position: 'absolute', top: 0, height: height, width: '100%', zIndex: 2 }}>
        <GestureDetector gesture={panGesture}>
          <Animated.View style={{ flex: 1 }}>
          </Animated.View>
        </GestureDetector>
      </GestureHandlerRootView>
      <Animated.View style={[{ height: 300, width: '100%', backgroundColor: 'red' }, animatedHeaderStyle]}>
        <Text>Sliding Header</Text>
      </Animated.View>
      <Animated.View style={[{ height: height - 50, backgroundColor: 'blue' }, animatedContentStyle]}>
        <Animated.ScrollView
          ref={scrollRef}
              ...
              'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('')
                .map((s, i) => {
                  return <TouchableOpacity key={i}>
                    <View style={{ padding: 5, backgroundColor: 'red', width: 50, alignItems: 'center', justifyContent: 'center' }}>
                      <Text style={{ textAlign: 'center', fontSize: 30, color: 'white' }}>{s}</Text>
                    </View>
                  </TouchableOpacity>
                })
            }
