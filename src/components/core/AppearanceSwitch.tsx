...
const AppearanceSwitch = () => {
  ...
  const [, setMode] = useState<string | null>(null);
  const dispatch = useDispatch();
  const autoSelectAppearance = useSelector<any>(
    state => state.appReducer.autoSelectAppearance,
  );
    ...
    if (!autoSelectAppearance) {
      setMode(state => {
        ...
        Animated.timing(posX.current, {
          ...
          if (finished) {
            dispatch(
              state === APP_APPEARANCE_TYPE.DARK
                ? setLightAppearance()
                : setDarkAppearance(),
            );
          }
        });
        return state === APP_APPEARANCE_TYPE.LIGHT
          ? APP_APPEARANCE_TYPE.DARK
          : APP_APPEARANCE_TYPE.LIGHT;
      });
    }
  }, [autoSelectAppearance, dispatch]);

  useEffect(() => {
    handleChangeAppearance = handleChangeMode;
    setMode(appearanceType);
  }, [appearanceType, handleChangeMode]);
    ...
    <TouchableOpacity onPress={handleChangeMode} activeOpacity={0.85}>
      <View
        style={{
          ...
          borderRadius: 20 / 2,
          overflow: 'hidden',
          opacity: autoSelectAppearance ? 0.5 : 1,
        }}>
        <Animated.View
          style={{
        ...
        <Animated.View
          ...
          <Animated.View
            ...
            <Icon name="light-mode" size={10} color={theme['warning-text']} />
          </Animated.View>
          <Animated.View
            style={{
              position: 'absolute',
            ...
            <Icon name="dark-mode" size={10} color={theme['primary-text']} />
          </Animated.View>
        </Animated.View>
      </View>
    </TouchableOpacity>
